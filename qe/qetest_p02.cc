...
RC privateTestCase_2() {
    ...
    *(int *) value2.data = compVal;
    ...
    while (filter1->getNextTuple(data1) != QE_EOF) {
        ...
        filter1->getAttributes(tmpattrs);
        RelationManager::instance().printTuple(tmpattrs, data1);
        ...
        filter2->getAttributes(tmpattrs);
        RelationManager::instance().printTuple(tmpattrs, data2);
    ...
    if (count != 40 || nullCount1 != 20 || nullCount2 != 20) {
        std::cerr << "***** [FAIL] here The number of result: " << count << " " << nullCount1 << " " << nullCount2
