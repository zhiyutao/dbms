...
int RBFTest_Update(RecordBasedFileManager &rbfm) {
    ...
    // self test case
    std::vector<RID> rids;
    for(int i = 0; i < 100; ++ i){ // insert 100 shortStr
        rids.emplace_back();
        insertRecord(rbfm, rids.back(), shortStr);
    }
    for(int i = 0; i < 50; ++ i){
        updateRecord(rbfm, rids[i], midString);
    }
    for(int i = 0; i < 50; ++ i){
        readRecord(rbfm, rids[i], midString);
    }
    for(int i = 50; i < 100; ++ i){
        readRecord(rbfm, rids[i], shortStr);
    }
